//
//  Name.swift
//  SocialMediaApp
//
//  Created by Grzegorz on 19/10/2017.
//  Copyright © 2017 Grzegorz Kwaśniewski. All rights reserved.
//

import Foundation

struct Name {
    
    static let emailLoginButton = "emailLoginButton"
    static let twitterLoginButton = "twitterLoginButton"
    
    static let signInLabel = "signInLabel"
    static let backButton = "backButton"
    static let loginWithEmailButton = "loginWithEmailButton"
    static let createAccountButton = "createAccountButton"
    static let dontHaveAccount = "dontHaveAccount"
    static let goBackToLoginScreen = "goBackToLoginScreen"
    static let errorMessage = "errorMessage"
    static let dismissButton = "dismissButton"
    static let emailField = "emailField"
    static let passwordField = "passwordField"
    
    
    static let RESULT_VIEW_MESSAGE = "resultViewMessage"
    static let CLEAR_BUTTON = "clearButton"
    static let CARD_NUMBER_FIELD = "cardNumberField"
    static let CARD_VALID_MONTH_FIELD = "cardValidMonthField"
    static let CARD_VALID_YEAR_FIELD = "cardValidYearField"
    static let CARD_CVC_FIELD = "cardCVCField"
    static let CARD_CONFIRMED = "Card confirmed"
    
}

